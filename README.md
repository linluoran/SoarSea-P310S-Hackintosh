...
使用打**补丁方式 Sonoma 后续每次更新都是全量**, 每次更新完可能需要重新打补丁
...
+ 本项目EFI已经做好USB定制  
+ 替换升级 Mac Sonoma 后 wifi 可以打开 **搜索不到wifi 是正常情况,  使用OCLP 打好补丁可以恢复正常**.
+ 目前本人使用 仅连接手机热点有异常 无法正常连接(老系统也有此问题)  双向隔空可以正常使用



## 使用步骤

**提前下载好 `OpenCore Legacy Patcher` 工具 安装**

1 下载项目中的EFI 替换本机, 清空一下 nvram 重启后测试是否功能正常. 不正常的话换回原本EFI 停止升级.

2  正常的话, 设置- 全量更新到 Sonoma 正式版, 测试版系统未经过测试 请勿尝试.

3 按照教程中使用 `OpenCore Legacy Patcher` 工具直接打好补丁重启就可以正常使用了 **如果遇到卡开机的可以按教程卸载后重复打补丁**
